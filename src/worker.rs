...
mod tests {
    ...
    async fn test_worker_creation() {
        ...
        let worker = Worker::new(
            ...
            100,
            (20, 1400),
            create_test_protocol_mix(),
            false,
            true, // dry run
        );

        assert_eq!(worker.task_id, 0);
        assert!(worker.dry_run);
    }

    #[tokio::test]
    async fn test_simulate_packet_send() {
        let stats = Arc::new(FloodStats::default());
        let running = Arc::new(AtomicBool::new(true));
        let multi_port_target = Arc::new(MultiPortTarget::new(vec![80]));
        let target_ip = IpAddr::V4(Ipv4Addr::new(192, 168, 1, 1));
        ...
        worker.simulate_packet_send(&packet_data, "UDP").await;

        // Should have incremented either sent or failed
